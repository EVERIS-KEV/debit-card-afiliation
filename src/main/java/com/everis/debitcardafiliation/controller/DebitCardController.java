...
public class DebitCardController {
  ...
  public Mono<Object> create(@RequestBody @Valid DebitCard model, BindingResult bindinResult) {
    ...
    return service.save(new DebitCard(model.getIdCustomer(), model.getPassword()));
  }

  @PostMapping("/addAccount/{id}")
  public Mono<Object> addAccount(@PathVariable("id") String id, @RequestBody @Valid AccountAffiliate model, BindingResult bindinResult) {
    if (bindinResult.hasErrors()) return errorResult(bindinResult);
  ...
  public Mono<Object> setPrincipal(@PathVariable("id") String id, @RequestBody @Valid AccountAffiliate model, BindingResult bindinResult) {
    if (bindinResult.hasErrors()) return errorResult(bindinResult);
  ...
  public Mono<Object> addMovements(@RequestBody @Valid MovementFrom model, BindingResult bindinResult) {
    ...
    return Mono.just(service.addMovement(new MovementFrom(model.getNumberDebitCard(), model.getPassword(), model.getAmount(), "")));
  }
}
