...
public class DebitCardService {
    ...
    private Boolean verifyCustomer(String id) {
        return Webclient.customer
            .get()
            .uri(Constants.PathService.VERIFY_CUSTOMER, id)
            .retrieve()
    ...
    private AccountMapper findCurrentAccountByNumber(String number) {
        return Webclient.currentAccount
            .get()
            .uri(Constants.PathService.NUMBER_ACCOUNT, number)
            .retrieve()
            .bodyToMono(AccountMapper.class)
            .block();
    }
        ...
        try {
            return (
                repository
                    .findAll()
                    .stream()
                    .filter(c -> c.getIdDebitCard().equals(id))
                    .collect(Collectors.toList())
                    .isEmpty() ||
                !verifyAccount(model.getNumberAccount()) ||
                !findByAccount(model.getNumberAccount())
                    .getIdCustomer()
    ...
    private MovementFrom authCard(MovementFrom model) {
        if (
            !repository
                ...
                .filter(
                    c ->
                        c.getCardNumber().equals(model.getNumberDebitCard()) &&
                        c.getPassword().equals(model.getPassword())
                )
                .collect(Collectors.toList())
                .isEmpty()
        ) {
    ...
    private void AddMovementAccount(String number, Double amount, String numberCard) {
        DebitCard card = repository
            .findAll()
            .stream()
            .filter(c -> c.getCardNumber().equals(numberCard))
            .findFirst()
            .get();
        ...
        if (verifyNumberSC(number)) {
            Webclient.savingAccount
                .post()
                .uri("/movememts")
                .body(Mono.just(model), MovementsMapper.class)
                .retrieve()
    ...
    public Mono<Object> addMovement(MovementFrom model) {
        try {
            DebitCard debitcard = repository
                .findAll()
                .stream()
                .filter(c -> c.getCardNumber().equals(model.getNumberDebitCard()))
                .findFirst()
                .get();
            ...
            if (accountPrincipal.getAmount() < amount) {
                ...
                if (amountAll < amount) {
                    ...
                    AddMovementAccount(
                        accountPrincipal.getAccountNumber(),
                        accountPrincipal.getAmount(),
                        model.getNumberDebitCard()
                    );
                    debitcard
                        .getAccounts()
                        .stream()
    ...
    public Mono<Object> get(String number) {
        try {
            return Mono.just(
                repository.findAll().stream().filter(c -> c.getCardNumber().equals(number)).findAny().get()
            );
        } catch (Exception e) {
