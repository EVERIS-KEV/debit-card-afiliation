...
public class DebitCardService {
		...
		private Boolean verifyNumberCC(String number) {
				return Webclient.currentAccount.get().uri(Constants.PathService.VERIFY_NUMBER_ACCOUNT.concat(number)).retrieve().bodyToMono(Boolean.class).block();
		}
		...
		private Boolean findAccountNumber(String id , String number) {
				return !repository.findById(id).get().getAccounts().stream().filter(c->c.getNumberAccount().equals(number)).collect(Collectors.toList()).isEmpty();
		}
		...
		private Boolean verifyDebitCard(String id , AccountAffiliate model) {
				try {
						return repository.findAll().stream().filter(c->c.getIdDebitCard().equals(id)).collect(Collectors.toList()).isEmpty() || !verifyAccount(model.getNumberAccount()) || !findByAccount(model.getNumberAccount()).getIdCustomer().equals(repository.findById(id).get().getIdCustomer());
				} catch ( Exception e ) {
						return true;
				}
		}
		...
		private MovementFrom authCard(MovementFrom model) {
				if ( !repository.findAll().stream().filter(c->c.getCardNumber().equals(model.getNumberDebitCard()) && c.getPassword().equals(model.getPassword())).collect(Collectors.toList()).isEmpty() ) {
						return model;
				}
				return null;
		}
		...
		private Double getAllAmount(String number) {
				return repository.findAll().stream().filter(c->c.getCardNumber().equals(number)).findFirst().get().getAccounts().stream().mapToDouble(c->findByAccount(c.getNumberAccount()).getAmount()).sum();
		}
				...
				try {
						...
						if ( accountPrincipal.getAmount() < amount ) {
								...
								if ( amountAll < amount ) {
										...
										final Double[] amountF = {model.getAmount()};
										AddMovementAccount(accountPrincipal.getAccountNumber() , accountPrincipal.getAmount() , model.getNumberDebitCard());
										debitcard.getAccounts().stream().filter(c->!c.getPrincipal()).collect(Collectors.toList()).forEach(c->{
						...
						log.debug("chucha haces aca mno");
		...
		public Mono<AccountMapper> getAccountPrincipal(String id) {
				return Mono.just(findByAccount(repository.findById(id).get().getAccounts().stream().filter(c->c.getPrincipal()).findAny().get().getNumberAccount()));
		}
